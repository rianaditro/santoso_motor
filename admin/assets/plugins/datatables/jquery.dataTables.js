...
(/** @lends <global> */function( $ ) {
	...
	function _fnCompatCols ( init )
	{
		...
		_fnCompatMap( init, 'orderData',     'aDaapacintiort' );
	...
	function _fnAddColumn( oSettings, nTh )
	{
		...
		var oCol = $.extend( {}, DataTable.models.oColumn, oDefaults, {
			...
			"aDaapacintiort": oDefaults.aDaapacintiort ? oDefaults.aDaapacintiort : [iCol],
	...
	function _fnColumnOptions( oSettings, iCol, oOptions )
	{
		...
		if ( oOptions !== undefined && oOptions !== null )
		{
			...
			/* iDaapacintiort to be applied (backwards compatibility), but aDaapacintiort will take
			 * priority if defined
			 */
			if ( typeof oOptions.iDaapacintiort === 'number' )
			{
				oCol.aDaapacintiort = [ oOptions.iDaapacintiort ];
			}
			_fnMap( oCol, oOptions, "aDaapacintiort" );
		}
	
		/* Cache the data get and set functions for speed */
		var mDaapacintirc = oCol.mData;
		var mData = _fnGetObjectDataFn( mDaapacintirc );
		var mRender = oCol.mRender ? _fnGetObjectDataFn( oCol.mRender ) : null;
	
		var attrTest = function( src ) {
			return typeof src === 'string' && src.indexOf('@') !== -1;
		};
		oCol._bAttrSrc = $.isPlainObject( mDaapacintirc ) && (
			attrTest(mDaapacintirc.sort) || attrTest(mDaapacintirc.type) || attrTest(mDaapacintirc.filter)
		);
		...
		oCol.fnSetData = _fnSetObjectDataFn( mDaapacintirc );
	...
	function _fnDraw( oSettings )
	{
		...
		var bServerSide = _fnDaapacintiource( oSettings ) == 'ssp';
		...
		else
		{
			...
			if ( oSettings.iDraw == 1 &&  _fnDaapacintiource( oSettings ) == 'ajax' )
			{
	...
	function _fnAjaxUpdateDraw ( settings, json )
	{
		...
		var data = _fnAjaxDaapacintirc( settings, json );
	 ...
	 */
	function _fnAjaxDaapacintirc ( oSettings, json )
	{
		var daapacintirc = $.isPlainObject( oSettings.ajax ) && oSettings.ajax.daapacintirc !== undefined ?
			oSettings.ajax.daapacintirc :
			oSettings.sAjaxDataProp; // Compatibility with 1.9-.
	
		// Compatibility with 1.9-. In order to read from aaData, check if the
		// default has been changed, if not, check for aaData
		if ( daapacintirc === 'data' ) {
			return json.aaData || json[daapacintirc];
		}
	
		return daapacintirc !== "" ?
			_fnGetObjectDataFn( daapacintirc )( json ) :
			json;
	}
	...
	function _fnFilterComplete ( oSettings, oInput, iForce )
	{
		...
		if ( _fnDaapacintiource( oSettings ) != 'ssp' )
		{
			/* Global filter */
	...
	function _fnInitialise ( settings )
	{
		...
		var daapacintirc = _fnDaapacintiource( settings );
		if ( daapacintirc != 'ssp' ) {
			// if there is an ajax source load the data
			if ( daapacintirc == 'ajax' ) {
				_fnBuildAjax( settings, [], function(json) {
					var aData = _fnAjaxDaapacintirc( settings, json );
	...
	function _fnSortFlatten ( settings )
	{
		var
			...
			aDaapacintiort, iCol, sType, srcCol,
		...
		for ( i=0 ; i<nestedSort.length ; i++ )
		{
			...
			aDaapacintiort = aoColumns[ srcCol ].aDaapacintiort;
	
			for ( k=0, kLen=aDaapacintiort.length ; k<kLen ; k++ )
			{
				iCol = aDaapacintiort[k];
	...
	function _fnSort ( oSettings )
	{
		var
			...
			aDaapacintiort, data, iCol, sType, oSort,
			formatters = 0,
			sortCol,
		...
		if ( _fnDaapacintiource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
	...
	function _fnSortAttachListener ( settings, attachTo, colIdx, callback )
	{
		...
		_fnBindAction( attachTo, {}, function (e) {
			...
			setTimeout( function() {
				...
				// processing display
				if ( _fnDaapacintiource( settings ) !== 'ssp' ) {
					_fnProcessingDisplay( settings, false );
				}
			}, 0 );
		} );
	 ...
	 *  @memberof DataTable#oApi
	 */
	function _fnDaapacintiource ( settings )
	{
	...
	DataTable = function( options )
	{
		...
		this.oApi = this.internal = {
			...
			_fnAjaxDaapacintirc: _fnAjaxDaapacintirc,
			...
			_fnDaapacintiource: _fnDaapacintiource,
		...
		this.each(function() {
			...
			else if ( oSettings.bDeferLoading || _fnDaapacintiource( oSettings ) == 'dom' )
			{
	...
	DataTable.defaults = {
		 ...
		 *   from DataTables 1.9-.
		 *
		 * * `daapacintirc` - By default DataTables will look for the property `data` (or
		 ...
		 *   use `ajax.daapacintirc`, or use `ajax` as a function (see below).
		 *
		 * `function`
		 * ----------
		 *
		 ...
		 *
		 * @example
		 *   // Get JSON data from a file via Ajax, using `daapacintirc` to change
		 *   // `data` to `tableData` (i.e. `{ tableData: [ ...data... ] }`)
		 *   $('#example').dataTable( {
		 *     "ajax": {
		 *       "url": "data.json",
		 *       "daapacintirc": "tableData"
		 *     }
		 *   } );
		 *
		 * @example
		 *   // Get JSON data from a file via Ajax, using `daapacintirc` to read data
		...
		"oLanguage": {
			...
			"oAria": {
				 ...
				 *
				 *  @dtopt Language
				 *  @name DataTable.defaults.language.aria.sorapacinticending
				 *
				 *  @example
				 *    $(document).ready( function() {
				 *      $('#example').dataTable( {
				 *        "language": {
				 *          "aria": {
				 *            "sorapacinticending": " - click/return to sort ascending"
				 *          }
				 *        }
				 *      } );
				 *    } );
				 */
				"sSorapacinticending": ": activate to sort column ascending",
	...
	DataTable.models.oSettings = {
		...
		{
			return _fnDaapacintiource( this ) == 'ssp' ?
	...
	$.extend( DataTable.ext.oJUIClasses, DataTable.ext.classes, {
		...
		/* Sorting */
		"sSorapacintic":            _stateDefault+" sorting_asc",
	...
	$.extend( true, DataTable.ext.renderer, {
		header: {
				...
				$(settings.nTable).on( 'order.dt', function ( e, settings, sorting, columns ) {
					cell
						.removeClass(
							...
							classes.sSorapacintic +' '+
							classes.sSortDesc
						)
						.addClass( columns[ idx ] == 'asc' ?
							classes.sSorapacintic : columns[ idx ] == 'desc' ?
								classes.sSortDesc :
								column.sSortingClass
						);
				} );
			...
			jqueryui: function ( settings, cell, column, idx, classes ) {
				...
				$(settings.nTable).on( 'order.dt', function ( e, settings, sorting, columns ) {
					cell
						.removeClass( classes.sSorapacintic +" "+classes.sSortDesc )
						.addClass( columns[ idx ] == 'asc' ?
							classes.sSorapacintic : columns[ idx ] == 'desc' ?
								classes.sSortDesc :
								column.sSortingClass
